...
class assign_content_problemsheet extends assign_content_plugin {
    ...
    /**
     * Get the time at which the problem sheet for this assignment will be available to students.
     *
     * The function returns a unix timestamp.
     * As a special values, "0" means immediate availability.
     */
    protected function get_problem_availability_time() {
        $time = $this->assignment->get_instance()->allowsubmissionsfromdate;
        return $time;
    }

    /**
     * Determine whether the current user can view problem sheets in the current context.
     *
     * @return boolean whether the current user can view problem sheets in the current context
     */
    public function can_view_problems() {
        $context = $this->assignment->get_context();
        $canview = false;
        if (has_capability('assigncontent/problemsheet:viewproblemanytime', $context)) {
            $canview = true;
        } else {
            $canview = $this->get_problem_availability_time() <= time();
        }
        return $canview;
    }
